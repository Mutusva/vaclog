...
// MakeHTTPRequest in the case of GET, the parameter queryParameters is transferred to the URL as query parameters
// in the case of POST, the parameter body, an io.Reader, is used
// Note: this could be moved to its own package but just keeping it simple.
func MakeHTTPRequest[T any](ctx context.Context, client *http.Client, fullUrl string, httpMethod string, headers map[string]string, queryParameters url.Values,
	body io.Reader, responseType T) (T, error) {
